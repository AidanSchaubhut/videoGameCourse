(function(window, createjs, opspark, _) {
  ...
  function update(event) {
    ...
    //ball.y 
    if(ball.x + ball.radius >= paddleCPU.x && ball.x + ball.radius >= paddleCPU.x + widthCPU && ball.y - ball.radius >= paddleCPU.y && ball.y + ball.radius <= heightCPU + paddleCPU.y){
        ball.xVelocity = -ball.xVelocity;
    } else if (ball.x - ball.radius < paddlePlayer.x && ball.x - ball.radius - paddlePlayer.x + widthPlayer && ball.y - ball.radius >= paddlePlayer.y && ball.y + ball.radius <= heightPlayer + paddlePlayer.y){
        ball.xVelocity = -ball.xVelocity;
    }

    //Reset after ball goes out and add score
    if(ball.x < 0){
      AIScore++;
      ball.x = canvas.width/2;
      ball.y = canvas.height/2;
